...
trait ArithmeticParser extends RegexParsers with Syntax {
  ...
  def integer:Parser[IntegerLiteral] = """-?\d+""".r ^^ { s => new IntegerLiteral(s.toInt) }
  ...
  def parenthesizedExpr = "(" ~ expression ~ ")" ^^ { case ("(" ~ e ~ ")") => e }

  def operand = (integer | parenthesizedExpr)

  def expression:Parser[Expression] = ( sum | minus | product | divide | integer | parenthesizedExpr | failure("error occured") )
